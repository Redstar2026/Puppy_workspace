WITH sw7 AS (
  SELECT ITEM, PAIS, DIVISION, CATEGORIA, PRECIO_COMP_LOW as precio_sw7
  FROM `wmt-k1-cons-data-users.k1_adhoc_tables.PGMB_PREMED_A0M1J1N`
  WHERE SW = 7 AND ANIO = 2026
  AND PRECIO_COMP_LOW IS NOT NULL
),
sw8 AS (
  SELECT ITEM, PAIS, DIVISION, CATEGORIA, PRECIO_COMP_LOW as precio_sw8
  FROM `wmt-k1-cons-data-users.k1_adhoc_tables.PGMB_PREMED_A0M1J1N`
