WITH base AS (
  SELECT PAIS, DIVISION, CATEGORIA, ITEM,
  ...
  WHERE ANIO = 2026 AND SW = 8
    AND DIVISION != 'MG Y TEXTIL'
    AND FACTOR_CALC IS NOT NULL
    AND PESO_CALC IS NOT NULL AND PESO_CALC > 0
  GROUP BY PAIS, DIVISION, CATEGORIA, ITEM
),
stats AS (
  SELECT PERCENTILE_CONT(pg, 0.25) OVER () AS q1,
         PERCENTILE_CONT(pg, 0.75) OVER () AS q3
  FROM base
),
bounds AS (
  SELECT MIN(q1) - 1.5*(MIN(q3)-MIN(q1)) AS lower_bound,
         MAX(q3) + 1.5*(MAX(q3)-MIN(q1)) AS upper_bound
  FROM stats
)
SELECT b.PAIS, b.DIVISION, b.CATEGORIA, b.ITEM,
  b.item_descrip, b.brand_name, b.total_rows,
  b.sum_factor_calc, b.sum_peso_calc, b.pg, b.avg_price_gap,
  CASE
